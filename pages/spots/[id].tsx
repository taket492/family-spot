    ...
    <div className="page-container py-4">
      ...
      {spot && (
        <div className="mt-4">
            ...
            <div className="space-y-4">
              ...
              {spot.images.length > 1 && (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {spot.images.slice(1, 7).map((image, index) => (
                    <div key={index} className="relative overflow-hidden aspect-square bg-gray-100 rounded-lg">
                      <OptimizedImage
                        src={image}
                        alt={`${spot.name}の画像 ${index + 2}`}
                        fill
                  ...
                  {spot.images.length > 7 && (
                    <div className="relative flex items-center justify-center overflow-hidden aspect-square bg-gray-200 rounded-lg">
                      <span className="text-gray-600 text-sm">+{spot.images.length - 7}枚</span>
                    </div>
                  )}
                </div>
              )}
          ...
          {/* Add image button for existing spots with images */}
          {spot.images && spot.images.length > 0 && (
            <button
              onClick={handleImageClick}
              disabled={uploading}
              className="flex items-center justify-center gap-2 mt-2 w-full px-4 py-2 text-sm text-gray-600 transition-colors bg-gray-100 rounded-lg hover:bg-gray-200 disabled:bg-gray-50 disabled:cursor-not-allowed"
            >
              {uploading ? (
                <>
